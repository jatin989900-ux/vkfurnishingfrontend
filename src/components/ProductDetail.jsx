...
const S = [
  ...
  '.gallery-close{position:absolute;top:14px;right:14px;width:34px;height:34px;background:rgba(0,0,0,0.5);border:none;border-radius:50%;color:#fff;font-size:14px;cursor:pointer;z-index:10;}',
  '.gallery-nav{position:absolute;top:50%;transform:translateY(-50%);width:36px;height:36px;background:rgba(0,0,0,0.4);border:none;border-radius:50%;color:#fff;font-size:20px;cursor:pointer;z-index:10;display:flex;align-items:center;justify-content:center;}',
  ...
  '.gdot{width:6px;height:6px;border-radius:50%;background:rgba(255,255,255,0.4);cursor:pointer;transition:all 0.2s;}',
  ...
  '.ext-video-btn{position:absolute;bottom:12px;right:12px;background:rgba(192,57,43,0.9);color:#fff;border:none;padding:6px 12px;border-radius:20px;font-size:11px;font-weight:700;cursor:pointer;z-index:10;}',
  ...
  '.pd-name{font-size:22px;font-weight:700;margin-bottom:6px;line-height:1.2;}',
  '.pd-cat{font-size:12px;color:#8888AA;font-weight:600;text-transform:uppercase;margin-bottom:12px;}',
  ...
  '.pd-ws{font-size:28px;font-weight:700;color:#1A6B3C;}',
  ...
  '.add-cart-btn{width:100%;padding:14px;background:#1C1C2E;color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:10px;}',
  '.view-cart-btn{width:100%;padding:14px;background:#C9973A;color:#1C1C2E;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:8px;}',
  '.pd-reg-btn{width:100%;padding:14px;background:#1C1C2E;color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;}',
].join('')
          ...
          <div className="thumbs">
            {slides.map((slide, i) => (
              <div key={i} className={'thumb' + (i === idx ? ' active' : '')} onClick={() => setIdx(i)}>
                {slide.type === 'video'
                  ? <span style={{ color: '#fff', fontSize: 12 }}>VID</span>
          ...
          {approved ? (
            <>
              <div className="pd-price-section">
                ...
                <div className="pd-moq">Min. order: {p.moq} pcs · Profit Rs.{p.mrp - p.wholesale_price}/pc</div>
              </div>
              {p.in_stock && (
              ...
              {added ? (
                <div className="cart-added">
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: '#8888AA' }}>{qty} pcs · Rs.{(p.wholesale_price * qty).toLocaleString()}</div>
                  </div>
                </div>
              ) : p.in_stock && (
            ...
            <div>
              <div className="pd-price-locked">
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#8888AA', lineHeight: 1.4 }}>Register and get approved to see pricing</div>
                </div>
              </div>
