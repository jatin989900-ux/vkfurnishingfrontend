...
.gallery-nav{position:absolute;top:50%;transform:translateY(-50%);width:36px;height:36px;background:rgba(0,0,0,0.4);border:none;border-radius:50%;color:#fff;font-size:16px;cursor:pointer;z-index:10;}
...
const EMOJI = {'Bedsheets':'Bed','Dohars':'Do','Comforters':'Co','Blankets':'Bl','Towels':'To','Quilts':'Qu','Bathrobes':'Ba','Bedcovers':'Be','Top Sheets':'Ts','Other':'Ot'}export default function ProductDetail({ product: p, approved, onClose, onRegister }) {
    ...
    <div className="pd-overlay" onClick={e => e.target.classList.contains('pd-overlay') && onClose()}>
      ...
      <div className="pd-modal">
        <div className="gallery" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          ...
          {p.video_url && <button className="ext-video-btn" onClick={()=>window.open(p.video_url,'_blank')}>Watch Video</button>}
        </div>
          ...
          <div className="thumbs">
            {slides.map((slide,i) => (
              <div key={i} className={`thumb ${i===idx?'active':''}`} onClick={()=>setIdx(i)}>
                {slide.type==='video' ? <span>V</span> : <img src={slide.url} alt="" />}
              </div>
            ))}
          </div>
        )}
